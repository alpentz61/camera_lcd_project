...
#define delay(ms) 				timer_sleep(ms);

/*
 *	SPI transaction framing
 * */
#define SPI_BEGIN_TRANSACTION() //Not needed with raspberry pi
 ...
 * Control Pins
 * */
#define RESET_HIGH()			HAL_GPIO_WritePin(GPIOA,RESET,GPIO_PIN_SET)
#define RESET_LOW()				HAL_GPIO_WritePin(GPIOA,RESET,GPIO_PIN_RESET)
#define DC_HIGH()           	HAL_GPIO_WritePin(GPIOA,DC,GPIO_PIN_SET)
#define DC_LOW()            	HAL_GPIO_WritePin(GPIOA,DC,GPIO_PIN_RESET)
#define SPI_CS_HIGH()			HAL_GPIO_WritePin(GPIOA,CS,GPIO_PIN_SET)
#define SPI_CS_LOW()			HAL_GPIO_WritePin(GPIOA,CS,GPIO_PIN_RESET)
...
bool begin(void)
{
    ...
    __SPI1_CLK_ENABLE();
    memset(&spi,0,sizeof(SPI_HandleTypeDef));
    spi.Instance = SPI1;
...
/**************************************************************************/
/*!
    @brief   Disables the peripheral operation
*/
/**************************************************************************/
void end(void) {
}
...
*/
/**************************************************************************/
void writePix(int16_t x, int16_t y, uint16_t color) {
...
/**************************************************************************/
/*!
   @brief  Read 8-bit data via SPI. 
   @returns One byte of data from SPI
*/
/**************************************************************************/
uint8_t spiRead() {
    uint8_t b;
    HAL_SPI_Receive(&spi, &b, 1, HAL_MAX_DELAY);
    return b;
}
...
/**************************************************************************/
/*!
   @brief  Write 16-bit data via SPI. 
   @param  s Half word of data to write over SPI
*/
/**************************************************************************/
void spiWrite16(uint16_t s) {
    ...
    bytes[0] = s >> 8;
    bytes[1] = s;
    HAL_SPI_Transmit(&spi, (uint8_t*)bytes, 2, HAL_MAX_DELAY);
}

/**************************************************************************/
/*!
   @brief  Write 32-bit data via SPI. 
   @param  w Word of data to write over SPI
*/
/**************************************************************************/
void spiWrite32(uint32_t w) {
    ...
    bytes[2] = w >> 8;
    bytes[3] = w;
    HAL_SPI_Transmit(&spi, (uint8_t*)bytes, 4, HAL_MAX_DELAY);
}

/**************************************************************************/
/*!
   @brief  Write a buffer of color values via SPI
   @param  c Buffer of color values
